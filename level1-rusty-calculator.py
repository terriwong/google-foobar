...
def answer(str):
    ...
    for i in range(0, len(str)):
        if str[i] == "*":
            multi_stack.append(str[i])
        elif str[i] == "+":
            add_stack.append(str[i])
            number_stack.extend(multi_stack)
            result.extend(number_stack)
            number_stack = []
            multi_stack = []
        else:
            number_stack.append(str[i])
    
    result.extend(number_stack)
    result.extend(multi_stack)
    result.extend(add_stack)
