...
def answer(population, x, y, strength):
    patient_Z_resistance = population[y][x]
    if strength < patient_Z_resistance:
        return population
    elif strength >= patient_Z_resistance:
        population[y][x] = -1
        queue = set()  
        infected = set()
        checked = set()
        ...
        while len(queue - infected) != 0:           
            next = queue.pop()
            checked.add(next)
            curr_y = next[0]
            curr_x = next[1]

            if population[curr_y][curr_x] <= strength:
                population[curr_y][curr_x] = -1
                infected.add((curr_y, curr_x))

                neighbors = [(curr_y, curr_x - 1), (curr_y, curr_x + 1), (curr_y - 1, curr_x), (curr_y + 1, curr_x)]
                for y, x in neighbors:
                    if is_valid(y, x, infected, population):
                        queue.add((y, x))

        return population

def is_valid(y, x, infected, population):
    if (y, x) in infected:
        return False
    elif x < 0 or x >= len(population[0]):
        return False
    elif y < 0 or y >= len(population):
        return False
    else:
        return True
